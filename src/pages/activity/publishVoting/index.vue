<style lang="scss" scoped>
  ...
  .add-v-l-act{
    width:60px;
    height:60px;
    background-image: url("https://i.loli.net/2018/09/03/5b8cc97484968.png");
    background-size: 100% 100%;
...
<div class="container">
  <div class="setting-header">
    <div  :class="{active:settingSelect==1}"  @click="selectSetting(1)">基本设置</div>
    <div  :class="{active:settingSelect==2}"  @click="selectSetting(2)">投票设置</div>
    ...
    <div class=""  v-if="settingSelect==1">
      <div class="base-info">
        ...
        <div>
          ...
          <div class="form-group" >
            <label >联系方式</label>
            <input type="text" maxlength="30" class="form-input" placeholder="请输入主办方的联系方式" v-model="base.contactInformation">
          </div>
          <div  class="form-group">
            <label>开始时间</label>
    ...
    <div class=""  v-if="settingSelect==2">
      <div >
        <div class="add-v-l-w"  v-for="(item,index) in list" :key="key">
          <div class="add-l-cover" @click="setBaseImgs(item)">
          ...
          <div class="add-l-right">
            ...
            <div class="detail" @click="addListText(item)">{{item.describes==''?'请输入100字以内的简段说明':item.describes}}</div>
        ...
        <div class="add-v-l-act" @click="addSettingList()"></div>
        <div class="form-group">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      setBaseImgs(model){
        wx.navigateTo({
          url:"/pages/activity/votingListCover/main?setting=2&width=300&height=360&itemNo="+model.itemNo
        })
      },
      addImagePrev(){
        wx.navigateTo({
          url:"/pages/activity/votingCover/main?setting=1&width=300&height=168"
        })
      },
      selectSetting(index){
      ...
      validate(){
        ...
        if( !base.placardUrl){
          this.showToast('none','请选择一张满意的海报哦')
          return
        }
        ...
        for(let i=0;i<list.length;i++){
          ...
          if(!itemModel.photoUrl){
            this.showToast('none',cid +'的主题图没有选择')
            return
          }
          ...
          if(!this.Ept(itemModel.describes) || this.Ept(itemModel.describes).length<5 ){
            this.showToast('none',cid+'的详情描述不能为空，或者小于5个字')
            return
          }
        }
      ...
      uploadCover(){
        ...
        if(Cover && Cover.indexOf('test-1251126190')<0){
      ...
      uploadListFile(){
        ...
        const uploadImgList = index=>{
          ...
          if( list[index] && list[index].photoUrl && list[index].photoUrl.indexOf('test-1251126190')<0 ){
            let photoUrl = list[index].photoUrl
            _this.uploadImage(filePath,function(res,localtion){
              if(res.statusCode==200){
                list[index].photoUrl = localtion
                start+=1
                uploadImgList(start)
              }else{
                _this.showModal('none','第'+(index+1)+'张上传失败')
              }
            },function(progress){
              let percent = progress.progress
              _this.showToast('none',percent+"%"+'第'+(index+1)+'/'+totalImg+'张')
            })
          }
        }
    ...
    mounted(){
      ...
      let Cover = wx.getStorageSync('votingListCover')
      let IndexCover = wx.getStorageSync('votingCover')
      this.list.forEach(item=>{
        if(item.itemNo===parseInt(Cover.itemNo)){
          item.photoUrl = Cover.photoUrl||''
        }
      })
      this.base.placardUrl = IndexCover.photoUrl||''
    }
  }
